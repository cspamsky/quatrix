...
export class SystemService {
  /**
   * Check if systemd is available and PID 1 is initialized correctly
   */
  ...
  async getTimezones(): Promise<string[]> {
    const hasSystemd = await this.isSystemdAvailable();
    if (!hasSystemd) {
      console.warn('[SYSTEM] systemd not found, using fallback timezones.');
